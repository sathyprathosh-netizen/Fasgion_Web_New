...
(function () {
  ...
  function initThemeSwitcher() {
    ...
    const savedTheme = localStorage.getItem('luxe-theme') || 'sunset';
  ...
  function injectThemeSwitcherUI() {
    if (document.querySelector('.theme-switcher')) return;

    const switcher = document.createElement('div');
    switcher.className = 'theme-switcher';
    switcher.innerHTML = `
      <button class="theme-btn sunset" data-theme="sunset" title="Instagram Sunset"></button>
      <button class="theme-btn electric" data-theme="electric" title="Electric Purple"></button>
      <button class="theme-btn midnight" data-theme="midnight" title="Midnight Noir"></button>
    `;
  ...
  function init() {
    ...
    initSparkles();
    initTiltEffect();
    injectThemeSwitcherUI();
    initThemeSwitcher();

    // Home-specific: text reveal stagger
    if (document.querySelector('.hero-title')) {
      document.querySelectorAll('.hero-title .reveal-line').forEach((line, i) => {
        line.style.animationDelay = (0.3 + i * 0.25) + 's';
        line.style.animation = `maskLift 1s var(--ease-luxury) ${0.3 + i * 0.25}s both`;
